...
def _get_stocks_triple_list_from_df(df:pd.DataFrame,date:str):
    df['日期'] = date
    df['tuple'] = df[['代码','日期','现价']].apply(tuple,axis=1)
    return(list(df.iloc[:,4]))
...
def read_eastmoney_wrong_file_as_string():
    with open('wrong_5min_data_eastmoney.txt', 'r',encoding='utf-8') as f:
        return f.read()
