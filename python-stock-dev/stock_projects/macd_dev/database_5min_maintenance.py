...
wrong_eastmoney_data = file_utils.read_eastmoney_wrong_file_as_string()
...
def group_fix_5min_stocks_data():
    ...
    failed_stock_list = update_stocks_5min_data(stock_code_list)
    while len(failed_stock_list) != len(stock_code_list):
        stock_code_list = [res[0] for res in failed_stock_list]
        log.info('fix round: {}'.format(stock_code_list))
        failed_stock_list = update_stocks_5min_data(stock_code_list)
    [print(res) for res in failed_stock_list]
