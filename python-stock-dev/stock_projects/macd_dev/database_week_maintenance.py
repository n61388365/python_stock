#################################
## created 2025-06
#################################


import sqlite3
import utils.net_utils as net_utils
import utils.database_utils as database_utils
import utils.file_utils as file_utils
from utils import log
import concurrent.futures
import pandas as pd
import time


def _save_stock_week_data(stock_code:str,proxy_flag:bool,vendor:str):
    try:
        # 判断表是否存在
        sql_cmd = f"SELECT count(*) FROM sqlite_master WHERE type='table' AND name = 'stock_{stock_code}_week';"
        with sqlite3.connect('stocks_week.db') as conn:
            cursor = conn.cursor()
            listOfTable = cursor.execute(sql_cmd).fetchall()
            if listOfTable[0][0]:
                log.error(f"stock_{stock_code}_day exist.")
                return stock_code, '未保存', '表已存在'
    except Exception as e:
        log.error(e)
    log.debug(f'从网络获取 {stock_code} 数据')
    if vendor == 'eastmoney':
        df = net_utils.get_his_close_price_eastmoney(stock_code,102,proxy_flag)
    else:
        df = net_utils.get_his_close_price_sina(stock_code,102,proxy_flag)
    if df.empty:
        log.error(f'网络获取 {stock_code} 数据失败')
        return stock_code, '未保存', f'网络获取数据失败'
    try:
        database_utils.save_A_stock_week_data(stock_code,df)
    except Exception as e:
        log.error(e)
        return stock_code, '未保存', '数据库错误'
    log.debug(f'保存 {stock_code} 数据成功')
    return stock_code, '保存成功', '数据保存成功'

def init_all_week_data():
    failed_list = []
    stock_code_list = file_utils.get_all_stocks_list()
    log.info(f"total {len(stock_code_list)} stocks")
    start_time = time.localtime()
    step = 40
    for seg in range(int(len(stock_code_list)/step)+1):
        if len(stock_code_list)-seg*step < step:
            seg_list = stock_code_list[seg*step:]
        else:
            seg_list = stock_code_list[seg*step:seg*step+step]
        log.info(f"round {seg} {seg_list}")

        with concurrent.futures.ThreadPoolExecutor() as executor:
            result = executor.map(lambda code: _save_stock_week_data(code,True,'eastmoney'),seg_list)

        for res in result:
            if res[1] != '保存成功':
                log.error(res)
                failed_list.append(res)
    
    for stock in failed_list:
        print(stock)

    log.info('start {}'.format(time.strftime('%H:%M:%S', start_time)))
    log.info('end of init week data')


if __name__ == '__main__':
    init_all_week_data()
